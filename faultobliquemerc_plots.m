function [x, y] = faultobliquemerc_plots(lambda, phi, lambda1, phi1, lambda2, phi2,iii,name)
...
% check sign of dlamb
dlambcheck = sign(cosd(dlamb));
lendlamb=size(dlamb,1);
  if dlambcheck(lendlamb) ~= dlambcheck(lendlamb-1)
      fprintf('dlamb sign is wrong! See probseg plot(s)')
      lon1=lambda1(1);
      lat1=phi1(1);
      lon2=lambda2(1);
      lat2=phi2(1);
      %parsave(sprintf('problem_segment%d.txt', iii),lon1, lat1, lon2, lat2);
      f.longs=[lon1,lon2];
      f.latits=[lat1,lat2];
      figure(iii)
      plot(f.longs,f.latits,'ro','MarkerSize',5,'MarkerFaceColor','r')
      title(name);
      tmps=['probseg',num2str(iii),'.fig'];
      %tmps=['probfault_',name,'.png'];
      saveas(gca,tmps);
  end
