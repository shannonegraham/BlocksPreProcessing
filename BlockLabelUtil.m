function [S, b, st] = BlockLabelUtil(s, b, st)
% Modified BLockLabel for the blocks preprocessing
    ...
    for i = 1:nblock
        ...
        intpoint = sum(bin);
            if intpoint == 0
                 ...
                 axis tight
                 hold on
                 plot(bco,bca,'LineWidth',5,'Color','r');
                 title('No Interior Point')
            end
            if intpoint > 1
                figure
                plot(lon, lat, '-k') 
                ...
                hold on
                plot(bco,bca,'LineWidth',5,'Color','r');
                title('More than One Interior Point')
            end
...
%          if sum(bin) > 1 % more than 1 interior point
%              figure(i)
%              plot(bco,bca,'MakerFaceColor','r','MarkerSize',3);
%              tmps=['blockinfo',num2str(i),'.png'];
%              saveas(gca,tmps)
%          elseif sum(bin) = 0 %no interior point
%              figure(i)
%              plot(bco,bca,'MakerFaceColor','r','MarkerSize',3);
%              tmps=['blockinfo',num2str(i),'.png'];
%              saveas(gca,tmps)
